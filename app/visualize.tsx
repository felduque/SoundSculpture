...
export default function VisualizeScreen() {
  ...
  useEffect(() => {
    ...
    const loadSculpture = async () => {
      if (params.sculptureId) {
        ...
        const selectedSculpture =
          sculptures.find((s) => s.id === params.sculptureId) || null;
        if (selectedSculpture) {
          setCurrentSculpture(selectedSculpture);
          generateShapesFromAudio(selectedSculpture);
          await loadSound(selectedSculpture.uri);
        }
      }
    };
  ...
  const generateShapesFromAudio = (sculpture: SoundSculpture) => {
  ...
  if (isLoading) {
    return (
      <SafeAreaView
        className="flex-1 px-5 pt-10 pb-5"
      ...
      >
        ...
        <View className="flex-1 justify-center items-center">
          <ThemedText type="default" className="opacity-50 text-center">
            Selecciona una escultura sonora desde la galería para visualizarla.
          </ThemedText>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ThemedText type="default" className="opacity-70 mt-4 text-center">
          Desliza para rotar la visualización
        </ThemedText>
      </View>
      ...
      <ThemedText type="default" className="opacity-70 text-center">
        La visualización 3D representa las frecuencias y amplitudes del sonido
        grabado. Cada forma representa diferentes características del audio.
      </ThemedText>
    </SafeAreaView>
  );
}
